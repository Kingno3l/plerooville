<?php
include("includes/pageheader.php")
?>

<section class="tf-section tf-faq">
    <div class="container">
        <div class="row">
            <div class="col-12">
                <div class="title-heading st-2">
                    <div class="sub-heading clr-pri-3 f-mulish">
                        <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="77" height="30" viewBox="0 0 77 30">
                            <g>
                                <image width="77" height="30" xlink:href="data:img/png;base64," />
                            </g>
                        </svg>
                        <span class="inner-sub st-1">Need Any Support</span>
                        <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="77" height="30" viewBox="0 0 77 30">
                            <g>
                                <image width="77" height="30" xlink:href="data:img/png;base64," />
                            </g>
                        </svg>
                    </div>
                    <h2 class="title clr-pri-2">We Are Here To Assist You With Any Inquiries Or Information You May Need.</h2>
                </div>
            </div>
            <div class="col-md-4">
                <div class="sc-faq wow fadeIn animated" data-wow-delay="0.3ms" data-wow-duration="1200ms">
                    <div class="image">
                        <img src="assets/images/common/sc-faq1.png" alt="Image">
                    </div>
                    <div class="content">
                        <h4>Learning Adventures</h4>
                        <p class="desc">
                            Embark on exciting Learning Adventures tailored just for you! Our dedicated team of educators and experts is here to support and guide you through a world of knowledge. <br> Explore fascinating subjects, engage in interactive lessons, and discover the joy of learning in a fun and engaging environment. <a href="#" class="fl-btn st-5">
                                <span class="inner">Contact Us</span>
                            </a>
                    </div>
                </div>
            </div>
            <div class="col-md-4">
                <div class="sc-faq active wow fadeIn animated" data-wow-delay="0.3ms" data-wow-duration="1200ms">
                    <div class="image">
                        <img src="assets/images/common/sc-faq2.png" alt="Image">
                    </div>
                    <div class="content">
                        <h4>News and Discoveries</h4>
                        <p class="desc">
                            Dive into the world of News and Discoveries designed especially for young learners like you! Stay updated with the latest happenings, fascinating facts, and incredible discoveries. From science and nature to arts and culture, ignite your curiosity and expand your horizons with our engaging and educational content. <a href="#" class="fl-btn st-5">
                                <span class="inner">Contact Us</span>
                            </a>
                    </div>
                </div>
            </div>
            <div class="col-md-4">
                <div class="sc-faq wow fadeIn animated" data-wow-delay="0.3ms" data-wow-duration="1200ms">
                    <div class="image">
                        <img src="assets/images/common/sc-faq3.png" alt="Image">
                    </div>
                    <div class="content">
                        <h4>Fun Programs and Events</h4>
                        <p class="desc">
                            Join the fun-filled Programs and Events created exclusively for you! From exciting workshops to captivating events, we offer a wide range of interactive activities that blend learning with entertainment. Explore new interests, make friends, and embark on memorable educational journeys with our vibrant community of young learners.
                        </p>
                        <a href="#" class="fl-btn st-5">
                            <span class="inner">Contact Us</span>
                        </a>
                    </div>
                </div>
            </div>
        </div>
    </div>
</section>

<section class="tf-section tf-contact">
    <div class="container">
        <div class="row">
            <div class="col-md-5">
                <div class="title-heading">
                    <div class="sub-heading clr-pri-3 f-mulish">
                        <span class="inner-sub">Contact Us</span>
                        <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="77" height="30" viewBox="0 0 77 30">
                            <g>
                                <image width="77" height="30" xlink:href="data:img/png;base64," />
                            </g>
                        </svg>
                    </div>
                    <h2 class="title clr-pri-2 m-b26">Feel Free To Contact Us
                        For More Info</h2>
                    <p class="f-mulish">Get in touch with us for more information! We're here to answer any questions you may have. Reach out to our friendly team and discover how we can assist you on your learning journey. Contact us today and let the adventure begin!</p>
                </div>
                <div class="wrap-contact">
                    <div class="fx adress">
                        <div class="icon"><svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="43" height="65" viewBox="0 0 43 65">
                                <image id="location" width="43" height="65" xlink:href="data:img/png;base64," />
                            </svg>
                        </div>
                        <div class="content">
                            <p>Address</p>
                            <h4>3 Plerooville Crescent, Bye-Pass, Karshi, Nassarawa Nigeria</h4>
                        </div>
                    </div>
                    <div class="fx Email">
                        <div class="icon"><svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="60" height="51" viewBox="0 0 60 51">
                                <image id="chat_1_" data-name="chat (1)" width="60" height="51" xlink:href="data:img/png;base64," />
                            </svg>
                        </div>
                        <div class="content">
                            <p>Email Address</p>
                            <h4>info@plerooville.com.ng</h4>
                        </div>
                    </div>
                    <div class="fx phone">
                        <div class="icon"><svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="60" height="60" viewBox="0 0 60 60">
                                <image id="phone-call" width="60" height="60" xlink:href="data:img/png;base64," />
                            </svg>
                        </div>
                        <div class="content">
                            <p>Hotline</p>
                            <h4>+234(0) 803 276 1556</h4>
                        </div>
                    </div>
                </div>
            </div>
            <div class="col-md-7">
                <div class="image text-right p-t12 wow fadeIn animated" data-wow-delay="0.3ms" data-wow-duration="1200ms">
                    <img src="assets/images/common/01-contact.jpg" alt="">
                </div>
            </div>
        </div>
    </div>
</section>

<section>
    <div class="tf-section map">
        <div class="flat-map">
            <iframe src="https://www.google.com/maps/embed?pb=!1m14!1m12!1m3!1d624.5283722051438!2d7.5767190559625845!3d8.81719608812142!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!5e0!3m2!1sen!2sng!4v1685042169236!5m2!1sen!2sng" width="1720" height="655" style="border:0px; visibility: visible; animation-name: fadeInUp;" allowfullscreen="" loading="lazy"></iframe>
        </div>
    </div>
</section>

<section class="tf-section tf-message">
    <div class="container-fluid">
        <div class="row">
            <div class="col-md-12">
                <div class="form-message">
                    <h2 class="heading">
                        Leave a Message
                    </h2>
                    <form action="https://themesflat.co/kinco/contact/contact-process4.php" method="post" id="commentform" class="comment-form">
                        <div class="fx flex-wrap">
                            <fieldset class="name">
                                <input type="text" placeholder="Full Name Here" required name="name" class="name" id="name">
                            </fieldset>
                            <fieldset class="email">
                                <input type="email" placeholder="Email Address" required name="mail" class="mail" id="mail">
                            </fieldset>
                            <fieldset class="phone">
                                <input type="number" placeholder="Phone Number" required name="number" class="number" id="number">
                            </fieldset>
                            <fieldset class="select-wrap" role="group">
                                <div class="select">
                                    <select name="subject" id="subject">
                                        <option value="Please Select">Subject</option>
                                        <option value="services1">Subject 01</option>
                                        <option value="services2">Subject 02</option>
                                    </select>
                                </div>
                            </fieldset>
                            <fieldset class="message">
                                <textarea placeholder="Write Message" rows="5" tabindex="4" name="messagewr2" class="messagewr2" id="messagewr2"></textarea>
                            </fieldset>
                            <div class="wrap-btn">
                                <button type="submit" class="fl-btn st-6"><span class="inner">Send message</span></button>
                            </div>
                        </div>
                    </form>
                </div>
            </div>
        </div>
    </div>
</section>

<footer id="footer">
    <section class="tf-subcribe">
        <div class="container">
            <div class="row">
                <div class="col-lg-6 col-md-12 col-12">
                    <div class="subcribe-wp">
                        <h2 class="title">Subscribe Our Newsletter</h2>
                        <p class="sub f-mulish">Subscribe to our newsletter and unlock a world of knowledge and fun! Join our community of young learners and parents for exciting educational content, engaging activities, and the latest learning updates. Stay informed and inspired!</p>
                    </div>
                </div>
                <div class="col-lg-6 col-md-12 col-12">
                    <div class="subcribe-form fx" id="subscribe-form">
                        <form action="#">
                            <input type="email" id="subscribe-email" placeholder="Email Address">
                            <button class="fl-btn st-7" id="subscribe-button"><span class="inner">Subscribe</span></button>
                        </form>
                    </div>
                </div>
            </div>
        </div>
    </section>
    <?php
    include("includes/pagefooter.php")
    ?>